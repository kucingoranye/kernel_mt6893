...
/* BITS_PER_TYPE() is not available in 4.14 */
#define NBITS_MIN	BITS_PER_LONG
...
/* bitmap_zalloc() is not available in 4.14 (added in 4.19) */
static inline unsigned long *
dbitmap_zalloc(unsigned int nbits, gfp_t flags)
{
...
{
	dmap->map = dbitmap_zalloc(NBITS_MIN, GFP_KERNEL);
	if (!dmap->map) {
		dmap->nbits = 0;
		return -ENOMEM;
	}
